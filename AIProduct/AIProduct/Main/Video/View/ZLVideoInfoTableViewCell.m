...
@interface ZLVideoInfoTableViewCell ()
...
@implementation ZLVideoInfoTableViewCell
...
//    _detailLabel.text = @"今天天气很热,提醒服务多喝水哦,记得通风...";
    ...
    [_inviteBtn setTitle:@"视频邀请" forState:(UIControlStateNormal)];
    _inviteBtn.titleLabel.font = AdaptedFontSize(28);
    _inviteBtn.layer.borderColor = HEXCOLORAndAlpha(0x11ccf9, 0.85).CGColor;
    _inviteBtn.layer.borderWidth = 1.0;
    _inviteBtn.layer.cornerRadius = 3;
    _inviteBtn.clipsToBounds = YES;
    _inviteBtn.layer.masksToBounds = YES;
    [_inviteBtn setTitleColor:HEXCOLORAndAlpha(0x11ccf9, 0.85) forState:(UIControlStateNormal)];
    [_inviteBtn mas_makeConstraints:^(MASConstraintMaker *make) {
       
        make.right.equalTo(self.contentView.mas_right).offset(-AdaptedWidth(48));
        make.centerY.equalTo(self.contentView);
        make.height.mas_equalTo(AdaptedHeight(48));
        make.width.mas_equalTo(AdaptedWidth(154));
