...
typedef NS_ENUM(NSInteger, ZLPagerStyle) {
    /**<  上侧为下划线   **/
    ZLPagerStyleBottomLine = 0,
    /**<  上侧为滑块   **/
    ZLPagerStyleSlideBlock = 1,
};

@protocol ZLPagerViewDelegate <NSObject>
@optional
- (BOOL)deallocVCsIfUnnecessary; /**< 只保留最近的5个控制器，释放其他控制器的空间，如果滑到对应位置在对其重新创建加载 **/
@end

@interface ZLPagerView : UIView
...
@property (weak, nonatomic) id<ZLPagerViewDelegate>delegate; /**< NinaPagerView代理 **/
